%%
%% NB: This file is compiled from original msgpack/msgpack-erlang files.
%% Below is original license.
%%
...
pack(List)  when is_list(List) ->
    pack_array(List);
%% NB: added to be consistent with jsx:encode which allows atoms as hash keys
pack(Atom) when is_atom(Atom) ->
    pack(atom_to_binary(Atom, latin1));
pack(Other) ->
    throw({badarg, Other}).
